...
interface Options {
    ...
    swap: (team1: any, team2: any, divisionId: any) => void;
    userData: any;
    decorator: Decorator;
...
(function ($) {
    ...
    function winnerBubblesWithThirdPlace(match: Match): boolean {
        const el = match.el;
        const winner = el.find('.team.win');
        winner.append('<div class="bubble">1st</div>');
        const loser = el.find('.team.lose');
        loser.append('<div class="bubble">2nd</div>');      
        const third_1 = match.round().prev().match(0).el.find('.team.lose');
        const fourth_1 = match.round().prev().match(1).el.find('.team.lose');
        ...
        for (var r = 0; r < rounds; r += 1) {
            ...
            for (var m = 0; m < matches; m += 1) {
                ...
                if (!(r === rounds - 1 && isSingleElimination) && !(r === rounds - 1 && skipGrandFinalComeback)) {                    
                    ...
                    const match = round.addMatch(teamCb, skipConsolationRound ? winnerBubblesWithThirdPlace : winnerBubbles);                    
    ...
    const JqueryBracket = function (opts: Options) {
        ...
        function mkMatch(round: Round, data: Array<TeamBlock>, idx: number,
            ...
            function teamElement(round: number, team: TeamBlock, isReady: boolean) {
                ...
                if (isNumber(score) && score > 0) {
                    ...
                    const checkmUnset = $('<span class="checkmark"><div class="checkmark_circle"></div></div></span>');
                ...
                if (name.indexOf("|") > 0) {
                    ...
                    const ntEl = $('<div>' + name.split("|")[0] + '</div><div class="schoolLabel">' + name.split("|")[1] + '</div>');
                    nEl.append(ntEl);
                }
                else
                ...
                if (!(team.name === null || /*!isReady ||*/ !opts.save) && opts.save) {
                    ...
                    var dragFct=  function (span) {
                       //const span = $(this);
                        const otherDraggedNodes = opts.el.find('.team[switched=0]');
        ...
        const height = data.teams.length * teamContainerHeight;
        ...
        if (opts.save) {
            topCon.css('width', rounds * 220 + 40);
        }
        else {
            topCon.css('width', rounds * 220 + 10);
        }
